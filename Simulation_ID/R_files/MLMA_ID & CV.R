...
template<-data.frame(whole_ests=rep(NA, n_reps), whole_SE=NA, whole_Tau2=NA, whole_ICC=NA, m1.1_ests=NA, m1.1_SE=NA, m1.1_Tau2=NA, m1.1_ICC=NA, m1.2_ests=NA, m1.2_SE=NA, m1.2_Tau2=NA, m1.2_ICC=NA, m2_ests=NA, m2_SE=NA, m2_Tau2=NA, m2_ICC=NA, m3_ests=NA, m3_SE=NA, m3_Tau2=NA, m3_ICC=NA)
...
for(p in 1:nrow(parameters)){
	...
	for(n in 1:n_reps){
		...
		while(sum(is.na(template_p[n,])) > 0){
			...
			model2_CV<-try(rma.mv(yi = yi_CV, V = 0, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n_CV, R=list(ID2=Vf_CV), Rscale=F), silent=T)
			...
			# Try fit the model
			model3_CV<-try(rma.mv(yi = yi_CV, V = vi_CV, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n_CV, R=list(ID2=Vf_CV), Rscale=F), silent=T)
