...
template<-data.frame(whole_ests_CV=rep(NA, n_reps), whole_SE_CV=NA, whole_Tau2_CV=NA, whole_ICC_CV=NA, m1.1_ests_CV=NA, m1.1_SE_CV=NA, m1.1_Tau2_CV=NA, m1.1_ICC_CV=NA, m1.2_ests_CV=NA, m1.2_SE_CV=NA, m1.2_Tau2_CV=NA, m1.2_ICC_CV=NA, m2_ests_CV=NA, m2_SE_CV=NA, m2_Tau2_CV=NA, m2_ICC_CV=NA, m3_ests_CV=NA, m3_SE_CV=NA, m3_Tau2_CV=NA, m3_ICC_CV=NA
                     ,whole_ests_ID=rep(NA, n_reps), whole_SE_ID=NA, whole_Tau2_ID=NA, whole_ICC_ID=NA, m1.1_ests_ID=NA, m1.1_SE_ID=NA, m1.1_Tau2_ID=NA, m1.1_ICC_ID=NA, m1.2_ests_ID=NA, m1.2_SE_ID=NA, m1.2_Tau2_ID=NA, m1.2_ICC_ID=NA, m2_ests_ID=NA, m2_SE_ID=NA, m2_Tau2_ID=NA, m2_ICC_ID=NA, m3_ests_ID=NA, m3_SE_ID=NA, m3_Tau2_ID=NA, m3_ICC_ID=NA)
...
for(p in 1:nrow(parameters)){
	...
	for(n in 1:n_reps){
		...
		while(sum(is.na(template_p[n,])) > 0){
			...
			# Fit the model to the whole data 
			model_w_ID<-try(rma.mv(yi = yi_ID, V = vi_ID, random=list(~1|Study, ~1|Effect), data=data_n_ID), silent=T)
			...
			drop_CV<-sample(seq(1, nrow(data_n), 1), round(nrow(data_n_CV) * parameters_p$proportion_lost))
			data_n_CV$Control.SD[drop_CV]<-NA
			data_n_CV$Treatment.SD[drop_CV]<-NA
			data_n_CV$Control.CV[drop_CV]<-NA
			data_n_CV$Treatment.CV[drop_CV]<-NA
			data_n_CV$yi_CV[drop_CV]<-NA
			data_n_CV$vi_CV[drop_CV]<-NA
			
			
			# Find the row IDs you want to have missing SDs and set SD, ID and effect sizes as NA
			drop_ID<-sample(seq(1, nrow(data_n_ID), 1), round(nrow(data_n_ID) * parameters_p$proportion_lost))
			data_n_ID$Control.SD[drop_ID]<-NA
			data_n_ID$Treatment.SD[drop_ID]<-NA
			data_n_ID$Control.ID[drop_ID]<-NA
			data_n_ID$Treatment.ID[drop_ID]<-NA
			data_n_ID$yi_ID[drop_ID]<-NA
			data_n_ID$vi_ID[drop_ID]<-NA
			...
			data_n_CV$yi_CV[drop_CV]<-log(data_n_CV$Treatment.Mean[drop_CV] / data_n_CV$Control.Mean[drop_CV]) + 0.5 * (mcv1^2 / data_n_CV$Treatment.n[drop_CV] - mcv2^2 / data_n_CV$Control.n[drop_CV])
			data_n_CV$vi_CV[drop_CV]<-mcv1^2/data_n_CV$Treatment.n[drop_CV] + mcv2^2/data_n_CV$Control.n[drop_CV] + mcv1^4/(2 * data_n_CV$Treatment.n[drop_CV]^2) + mcv2^4/(2 * data_n_CV$Control.n[drop_CV]^2)
			
			
			# Now estimate a new yi and vi using the average ID where data are missing
			data_n_ID$yi_ID[drop_ID]<-log(data_n_ID$Treatment.Mean[drop_ID] / data_n_ID$Control.Mean[drop_ID]) + 0.5*((mid1/(data_n_ID$Treatment.Mean[drop_ID]*data_n_ID$Treatment.n[drop_ID])) - (mid2/(data_n_ID$Control.Mean[drop_ID]*data_n_ID$Control.n[drop_ID]))) ###equation 4 ou 6 Nakagawa et al.(2023)

			data_n_ID$vi_ID[drop_ID]<-((data_n_ID$Treatment.ID[drop_ID]) /data_n_ID$Treatment.Mean[drop_ID]*data_n_ID$Treatment.n[drop_ID]) + ((data_n_ID$Control.ID[drop_ID]) / data_n_ID$Control.Mean[drop_ID]*data_n_ID$Control.n[drop_ID]) +
			  ((data_n_ID$Treatment.ID[drop_ID]/data_n_ID$Treatment.Mean[drop_ID])^2 / (2*data_n_ID$Treatment.n[drop_ID]^2)) + ((data_n_ID$Control.ID[drop_ID]/data_n_ID$Control.Mean[drop_ID])^2 / (2*data_n_ID$Control.n[drop_ID]^2))
			# Fit the model
			...
			# Fit the model
			model1.2_CV<-try(rma.mv(yi = yi_CV, V = vi_2_CV, random=list(~1|Study, ~1|Effect), data=data_n_CV), silent=T)
			...
			# Create a matrix with v_tilda in the diagonal (note this is NOT the mix v_tilda and v_i)
			# Vf_ID<-diag((data_n_ID$Treatment.ID /data_n_ID$Treatment.Mean*data_n_ID$Treatment.n) + ((data_n_ID$Control.ID) / data_n_ID$Control.Mean*data_n_ID$Control.n) +
			#               ((data_n_ID$Treatment.ID)/data_n_ID$Treatment.Mean)^2 / (2*data_n_ID$Treatment.n^2)) + ((data_n_ID$Control.ID/data_n_ID$Control.Mean)^2 / (2*data_n_ID$Control.n^2))
			# 
			...
			#Note du groupe PSC: We encounterd the convergence problem
			...
			model2_ID<-try(rma.mv(yi = yi_ID, V = 0, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n_ID, R=list(ID2=Vf_ID), Rscale=F), silent=T)
			...
			# Add 0s to V matrix where SDs are known
			diag(Vf_ID)[-drop_ID]<-0
			
			# Now add 0s back to the vi where SDs are missing to remove v.tilda from where SD is missing
			data_n_ID$vi_ID[drop_ID]<-0
			

			# Try fit the model
			model3_CV<-try(rma.mv(yi = yi_CV, V = vi_CV, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n_CV,control=list(rel.tol=1e-8), R=list(ID2=Vf_CV), Rscale=F), silent=T)
			...
			# Try fit the model
			model3_ID<-try(rma.mv(yi = yi_ID, V = vi_ID, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n_ID, R=list(ID2=Vf_ID), Rscale=F), silent=T)
...
save(results, file=paste0("Simulation_MLMA_CV & ID", index, ".Rdata"))
save(parameters, file="Parameters_MLMA_CV & ID.Rdata")
