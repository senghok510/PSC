...
method_cols <- colorblind_pal()(10)  # Utilise maintenant 10 couleurs


method_cols <- c("color1", "color2", "color3", "color4", "color5", "color6", "color7", "color8", "color9", "color10")
...
A<-ggplot(data=agg_results, aes(y=median_bias, x=Method, col=Method, fill=Method)) +
  ...
  theme(legend.position="none", axis.title.y=element_text(size=15), axis.text.y=element_text(size=15), axis.text.x=element_text(size=15), plot.title=element_text(size=title_size)) +
  geom_hline(yintercept=0, col="dark grey", size=0.5) +
  scale_x_discrete(labels=c("Full_Data_with_cv", "Missing_Cases_with_cv", "All_cases_with_cv", "Multiplicative_with_cv", 
                            "Hybrid_with_id","Full_Data_with_id", "Missing_Cases_with_id", "All_cases_with_id", "Multiplicative_with_id", "Hybrid_with_id")) + scale_color_manual(values=method_cols) + scale_fill_manual(values=method_cols)
...
dim(cor.mat)
rownames(cor.mat)<-c("Full_Data_with_cv","Full_Data_with_id" , "Missing_Cases_with_cv", "Missing_Cases_with_id","All_cases_with_cv", "All_cases_with_id", "Multiplicative_with_cv", "Multiplicative_with_id","Hybrid_with_cv", "Hybrid_with_id")
colnames(cor.mat)<-c("Full_Data_with_cv","Full_Data_with_id" , "Missing_Cases_with_cv", "Missing_Cases_with_id","All_cases_with_cv", "All_cases_with_id", "Multiplicative_with_cv", "Multiplicative_with_id","Hybrid_with_cv", "Hybrid_with_id")
...
df_ID<-data.frame(delta=abs(wide[,"median_bias.Method 1.1_ID"]) - abs(wide[,"median_bias.Method 1.2_ID"]))
C_ID<-ggplot(df, aes(x=delta)) + geom_histogram(bins=20) + theme_bw() + geom_vline(xintercept=0, col="red") +
  xlab("Difference in |Bias|
...
dat1.1_CV<-agg_results[which(agg_results$Method=="Method 1.1_CV"),]
dat1.1_ID<-agg_results[which(agg_results$Method=="Method 1.1_ID"),]
dat1.2_CV<-agg_results[which(agg_results$Method=="Method 1.2_CV"),]
dat1.2_ID<-agg_results[which(agg_results$Method=="Method 1.2_ID"),]
...
grid.arrange(A+labs(title="A."), B+labs(title="B."), C+labs(title="C."), D+labs(title="D."), E_CV+labs(title="E_CV"),  E_ID+labs(title="E_ID"), 
